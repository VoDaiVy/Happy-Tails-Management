  ...
  return (
    <>
        ...
        >
          ...
          <Link to="/" className="flex items-center gap-2.5 group select-none flex-shrink-0">
            <div className="relative flex-shrink-0">
              ...
              >
                <PawPrint className="text-white transition-all duration-500" size={isScrolled ? 15 : 18} />
              </div>
              <span
            ...
            {user ? (
              <div className="relative" style={{ zIndex: 9999 }} ref={dropdownRef}>
                ...
                >
                  ...
                  <ChevronDown size={14} className={`transition-all duration-300 ${isScrolled ? 'text-white/50' : 'text-slate-400'} ${isDropdownOpen ? 'rotate-180' : ''}`} />
                </button>
      ...
      <div className={`fixed inset-0 z-40 md:hidden transition-all duration-300 ${isMobileMenuOpen ? 'visible' : 'invisible'}`}>
        ...
        />
        <div className={`absolute top-0 right-0 h-full w-72 bg-white shadow-2xl transition-transform duration-300 ease-out ${isMobileMenuOpen ? 'translate-x-0' : 'translate-x-full'}`}>
          <div className="flex items-center justify-between px-5 py-4 border-b border-slate-100">
            <div className="flex items-center gap-2.5">
              ...
              <span className="font-black text-slate-900 text-[17px]">Happy<span className="text-[#FF8C42]">Tails</span></span>
            </div>
            <button onClick={() => setIsMobileMenuOpen(false)} className="p-1.5 rounded-lg hover:bg-slate-100 text-slate-500">
              <X size={20} />
            </button>
          </div>
