...
const STATUS_TABS_STAFF = [
  { key: "pending", label: "Chờ nhận", icon: AlertCircle, color: "text-amber-500" },
  { key: "confirmed", label: "Đã nhận", icon: CheckCircle, color: "text-blue-500" },
  { key: "in-progress", label: "Đang thực hiện", icon: PlayCircle, color: "text-purple-500" },
  { key: "completed", label: "Hoàn thành", icon: CheckCircle, color: "text-green-500" },
];

const STATUS_TABS_ADMIN = [
  { key: "all", label: "Tất cả", icon: LayoutGrid, color: "text-gray-600" },
  { key: "pending", label: "Chờ xác nhận", icon: AlertCircle, color: "text-amber-500" },
  { key: "confirmed", label: "Đã xác nhận", icon: CheckCircle, color: "text-blue-500" },
  { key: "in-progress", label: "Đang thực hiện", icon: PlayCircle, color: "text-purple-500" },
  { key: "completed", label: "Hoàn thành", icon: CheckCircle, color: "text-green-500" },
  { key: "cancelled", label: "Đã hủy", icon: XCircle, color: "text-red-500" },
];

const BookingBoard = () => {
  const location = useLocation();
  
  // Xác định role từ path
  ...
  const fetchBookings = useCallback(async (showRefreshSpinner = false) => {
    try {
      ...
      // Staff chỉ thấy: đơn chưa ai nhận (pending) HOẶC đơn mình đã nhận
      if (role === "staff" && currentUserId) {
        fetchedBookings = fetchedBookings.filter((booking) => {
          // Đơn pending chưa ai nhận
          if (booking.status === "pending" && !booking.assignedStaff) {
            return true;
          }
          // Đơn mình đã nhận (bất kỳ trạng thái nào)
          if (
  ...
  // Handle claim booking (Staff nhận đơn)
  ...
  const handleAssignStaff = async (bookingId, staffId) => {
    try {
      ...
      alert(err.response?.data?.message || "Không thể gán nhân viên");
    }
  };
        ...
        <div>
          ...
          <p className="text-gray-500 text-sm mt-1">
            {role === "staff"
              ...
              : "Xem và giám sát toàn bộ đơn đặt lịch"}
          </p>
        </div>
      ...
      {loading ? (
        <div className="flex items-center justify-center py-20">
          <div className="flex flex-col items-center gap-4">
            ...
            <p className="text-gray-500">Đang tải danh sách đặt lịch...</p>
          </div>
        </div>
      ) : error ? (
        <div className="flex items-center justify-center py-20">
            ...
            <div>
              ...
              <p className="text-sm text-gray-500 mt-1">
                {searchQuery || selectedDate
                  ? "Thử thay đổi bộ lọc"
                  : role === "staff"
                  ? "Đơn mới sẽ hiển thị tại đây"
                  : "Đơn đặt lịch sẽ hiển thị tại đây"}
              </p>
            </div>
          </div>
        </div>
          ...
          <div className={`grid gap-4 ${role === "admin" ? "grid-cols-2 md:grid-cols-5" : "grid-cols-2 md:grid-cols-4"}`}>
            <div className="text-center p-3 bg-amber-50 rounded-xl">
              ...
              <p className="text-xs text-amber-600/80">
                {role === "staff" ? "Chờ nhận" : "Chờ xác nhận"}
              </p>
            </div>
            <div className="text-center p-3 bg-blue-50 rounded-xl">
              ...
              <p className="text-xs text-blue-600/80">
                {role === "staff" ? "Đã nhận" : "Đã xác nhận"}
              </p>
            </div>
            <div className="text-center p-3 bg-purple-50 rounded-xl">
              ...
              <p className="text-xs text-purple-600/80">Đang thực hiện</p>
            </div>
            ...
            {role === "admin" && (
              <div className="text-center p-3 bg-red-50 rounded-xl">
                ...
                <p className="text-xs text-red-600/80">Đã hủy</p>
              </div>
            )}
          </div>
        </div>
