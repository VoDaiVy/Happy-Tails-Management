...
const POLICY_ICON_MAP = {
  ...
  "life-buoy": MessageCircle,
  stethoscope: Stethoscope,
  home: Home,
  "book-open": CalendarCheck,
};

const parseStructuredPolicyContent = (rawContent) => {
  if (typeof rawContent !== "string") return null;
  try {
    const parsed = JSON.parse(rawContent);
    if (!parsed || typeof parsed !== "object") return null;
    if (parsed.display || parsed.faq || parsed.hero || parsed.support) {
      return parsed;
    }
  } catch {
    return null;
...
const makePolicyBullets = (structuredContent, fallbackText) => {
  const bullets = structuredContent?.content?.bullets;
  if (Array.isArray(bullets) && bullets.length > 0) {
    return bullets.filter(Boolean);
  }

  const chunks = [
    structuredContent?.content?.summaryText,
    structuredContent?.content?.detailedContent,
    fallbackText,
  ]
    .filter(Boolean)
    .join("\n")
...
const toSortedPolicies = (policies) =>
  [...policies].sort((left, right) => {
    const leftOrder = Number(left.display?.displayOrder || 9999);
    const rightOrder = Number(right.display?.displayOrder || 9999);
    return leftOrder - rightOrder;
  });

const mapPublicPolicySections = (apiPolicies) => {
  const mappedSections = {
    ...
    safety: [],
    faq: [],
    support: { ...DEFAULT_SUPPORT_CONTENT },
  };
  ...
  const normalized = toSortedPolicies(
    apiPolicies.map((policy) => {
      const structured = parseStructuredPolicyContent(policy.content);
      ...
      const iconKey = display.iconKey || structured?.iconKey || "sparkles";

      return {
        id: policy.slug || policy._id,
        title: policy.title,
        shortDescription: structured?.shortDescription || "",
        targetSection: display.targetSection || "detailed-guidelines",
        policyType: structured?.policyType || "accordion",
        display,
        content,
        faq,
        hero,
        support,
        icon: POLICY_ICON_MAP[iconKey] || Sparkles,
      };
    }),
  );

  const heroPolicy = normalized.find(
    (policy) =>
      policy.targetSection === "hero" && policy.policyType === "hero-content",
  );
  if (heroPolicy) {
    mappedSections.heroContent = {
      ...mappedSections.heroContent,
      eyebrowLabel: heroPolicy.hero.eyebrowLabel || "",
      title: heroPolicy.hero.title || heroPolicy.title || "",
      description:
        heroPolicy.hero.description || heroPolicy.shortDescription || "",
      ctaPrimaryText: heroPolicy.hero.ctaPrimaryText || "",
      ctaPrimaryLink: heroPolicy.hero.ctaPrimaryLink || "",
      ctaSecondaryText: heroPolicy.hero.ctaSecondaryText || "",
      ctaSecondaryLink: heroPolicy.hero.ctaSecondaryLink || "",
    };
  }

  const overviewPolicies = normalized.filter(
    (policy) => policy.targetSection === "policy-overview",
  );
  if (overviewPolicies.length > 0) {
    mappedSections.overview = overviewPolicies.map((policy) => ({
      id: policy.id,
      title: policy.content.cardTitle || policy.title,
      icon: policy.icon,
      desc: policy.content.cardDescription || policy.shortDescription || "",
    }));
  }

  const accordionPolicies = normalized.filter(
    (policy) => policy.targetSection === "detailed-guidelines",
  );
  if (accordionPolicies.length > 0) {
    mappedSections.accordion = accordionPolicies.map((policy) => ({
      id: policy.id,
      title: policy.title,
      icon: policy.icon,
      items: makePolicyBullets(policy, policy.content?.detailedContent),
    }));
  }

  const healthPolicy = normalized.find(
    (policy) => policy.targetSection === "pet-health-side-card",
  );
  const reminderPolicy = normalized.find(
    (policy) => policy.targetSection === "quick-reminder",
  );

  if (healthPolicy || reminderPolicy) {
    const nextQuickNotices = [];

    if (healthPolicy) {
      nextQuickNotices.push({
        id: "health-req",
        title: healthPolicy.content.cardTitle || healthPolicy.title,
        icon: healthPolicy.icon || ShieldCheck,
        items: makePolicyBullets(
          healthPolicy,
          healthPolicy.content?.detailedContent,
        ),
      });
    }
    ...
    if (reminderPolicy) {
      nextQuickNotices.push({
        id: "quick-reminder",
        title: reminderPolicy.content.cardTitle || reminderPolicy.title,
        icon: reminderPolicy.icon || Sparkles,
        desc:
          reminderPolicy.content.cardDescription ||
          reminderPolicy.shortDescription ||
          reminderPolicy.content.summaryText ||
          "",
      });
    }

    mappedSections.quickNotices = nextQuickNotices;
  }

  const safetyPolicies = normalized.filter(
    (policy) => policy.targetSection === "safety-promise",
  );
  if (safetyPolicies.length > 0) {
    mappedSections.safety = safetyPolicies.map((policy) => ({
      id: policy.id,
      title: policy.content.cardTitle || policy.title,
      icon: policy.icon,
      description:
        policy.content.cardDescription || policy.shortDescription || "",
    }));
  }

  const faqPolicies = normalized.filter(
    (policy) => policy.targetSection === "faq" && policy.policyType === "faq",
  );
  if (faqPolicies.length > 0) {
    mappedSections.faq = faqPolicies.map((policy) => ({
      id: policy.id,
      question: policy.faq.question || policy.title,
      answer: policy.faq.answer || policy.content?.detailedContent || "",
    }));
  }

  const supportPolicy = normalized.find(
    (policy) =>
      policy.targetSection === "support-cta" &&
      policy.policyType === "support-cta",
  );
  if (supportPolicy) {
    mappedSections.support = {
      ...mappedSections.support,
      title: supportPolicy.support.title || supportPolicy.title,
      description:
        supportPolicy.support.description ||
        supportPolicy.shortDescription ||
        "",
      liveChatLabel: supportPolicy.support.liveChatLabel || "",
      emailLabel: supportPolicy.support.emailLabel || "",
      callLabel: supportPolicy.support.callLabel || "",
    };
  }
...
const Policy = () => {
  ...
  const [policyOverview, setPolicyOverview] = useState([]);
  const [policyAccordion, setPolicyAccordion] = useState([]);
  const [quickNotices, setQuickNotices] = useState([]);
  const [safetyPromises, setSafetyPromises] = useState([]);
  const [faqItems, setFaqItems] = useState([]);
  const [supportContent, setSupportContent] = useState(DEFAULT_SUPPORT_CONTENT);
  ...
  useEffect(() => {
    const loadPublicPolicyContent = async () => {
      ...
      try {
        ...
        const mapped = mapPublicPolicySections(policies);

        setHeroContent(mapped.heroContent);
        setPolicyOverview(mapped.overview);
        setPolicyAccordion(mapped.accordion);
        setQuickNotices(mapped.quickNotices);
        setSafetyPromises(mapped.safety);
        setFaqItems(mapped.faq);
        setSupportContent(mapped.support);
      } catch (error) {
        setPolicyLoadError(
          "Unable to load policies from server. Please try again later.",
        );
        ...
        setSafetyPromises([]);
        setFaqItems([]);
        setSupportContent(DEFAULT_SUPPORT_CONTENT);
        console.error("Failed to load public policy content:", error);
      } finally {
        setIsLoadingPolicies(false);
      }
    };

    loadPublicPolicyContent();
  }, []);

  useEffect(() => {
    if (policyAccordion.length === 0) {
      setOpenPolicyId("");
      return;
    }

    if (!policyAccordion.some((item) => item.id === openPolicyId)) {
      setOpenPolicyId(policyAccordion[0].id);
    }
  }, [policyAccordion, openPolicyId]);
  ...
  useEffect(() => {
    ...
    if (!faqItems.some((item) => item.id === openFaqId)) {
  ...
  const hasSupportContent = Boolean(
    ...
    supportContent.liveChatLabel ||
    supportContent.emailLabel ||
    supportContent.callLabel,
  );
  ...
  const hasPolicyContent = Boolean(
    ...
    policyAccordion.length ||
    quickNotices.length ||
    safetyPromises.length ||
    faqItems.length ||
    hasSupportContent,
  );

  const handleSupportClick = (event, href = "#support") => {
      ...
      <main className="relative bg-white pb-10">
        <section className="relative px-4 pb-20 pt-32 sm:px-6 lg:pb-32 lg:pt-40 min-h-[650px] flex items-center">
          ...
          <div className="relative z-10 mx-auto grid w-full max-w-5xl items-center gap-12 lg:grid-cols-[1.1fr_0.9fr] px-4 lg:px-8">
            <div className="policy-fade space-y-6">
              ...
              {heroContent.ctaPrimaryText || heroContent.ctaSecondaryText ? (
                <div className="flex flex-wrap items-center gap-4 pt-2">
                  {heroContent.ctaPrimaryText && heroContent.ctaPrimaryLink ? (
                    <Link
                      to={heroContent.ctaPrimaryLink}
                  ...
                  {heroContent.ctaSecondaryText ? (
                    <a
                      href={heroContent.ctaSecondaryLink || "#support"}
                      onClick={(event) =>
                        handleSupportClick(
                          event,
                          heroContent.ctaSecondaryLink || "#support",
                        )
                      }
        ...
        {isLoadingPolicies ? (
          <section className="px-4 sm:px-6">
            <div className="mx-auto w-full max-w-6xl rounded-2xl border border-[color:var(--border)] bg-white px-4 py-6 text-sm text-[color:var(--text)] shadow-sm">
              Loading policy content from server...
            </div>
          </section>
        ) : null}

        {!isLoadingPolicies && policyLoadError ? (
          <section className="px-4 sm:px-6">
            <div className="mx-auto w-full max-w-6xl rounded-2xl border border-[#F6D5D9] bg-[#FFF5F6] px-4 py-6 text-sm font-medium text-[#B8273F] shadow-sm">
              {policyLoadError}
            </div>
          </section>
        ) : null}

        {!isLoadingPolicies && !policyLoadError && !hasPolicyContent ? (
          <section className="px-4 sm:px-6">
            <div className="mx-auto w-full max-w-6xl rounded-2xl border border-[color:var(--border)] bg-white px-4 py-8 text-center shadow-sm">
              <p className="text-lg font-bold text-[color:var(--navy)]">
                No published policy data available.
              </p>
              <p className="mt-1 text-sm text-[color:var(--text)]">
                Please publish policy items from the admin page to display them
                here.
              </p>
            </div>
          </section>
                ...
                {policyOverview.map((item, index) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      >
                        {React.createElement(item.icon || Sparkles, {
                          size: 24,
                          strokeWidth: 2.5,
                        })}
                      </div>
        ...
        {policyAccordion.length > 0 || quickNotices.length > 0 ? (
          <section
                ...
                <div className="space-y-6 sm:space-y-8">
                  <div>
                    <h2 className="text-2xl font-extrabold tracking-tight text-[color:var(--navy)] sm:text-3xl relative inline-block">
                      Detailed Guidelines
                      <svg
                        className="absolute -bottom-3 left-0 w-16 text-[color:var(--orange)]"
                        viewBox="0 0 100 10"
                    ...
                    <p className="mt-4 text-sm sm:text-base leading-relaxed text-[color:var(--text)] opacity-80 max-w-xl">
                      Everything you need to know to ensure a smooth, pleasant
                  ...
                  <div className="relative rounded-[2rem] bg-white p-2 sm:p-4 shadow-[0_15px_40px_-15px_rgba(30,42,58,0.06)] ring-1 ring-[color:var(--border)] overflow-visible">
                    ...
                    <div className="flex flex-col relative z-10 gap-1.5 sm:gap-2">
                      {policyAccordion.length === 0 ? (
                        <p className="px-4 py-6 text-sm text-[color:var(--text)] opacity-80">
                          No detailed policy guidelines published.
                        </p>
                      ) : (
                        policyAccordion.map((section, index) => {
                          ...
                          return (
                            ...
                            >
                              ...
                              >
                                <div className="flex items-center gap-3 sm:gap-4 flex-1">
                                  ...
                                  <div>
                                    <div className="flex items-center gap-2">
                                      <h3
                                        className={`text-sm sm:text-base transition-colors font-bold ${
                                          isActive
                                ...
                                <div
                                  className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-full transition-all duration-300 ${
                                    isActive
                                      ? "bg-[rgba(255,138,76,0.15)] text-[color:var(--orange)] rotate-180"
                                      : "bg-[#F3F5F7] text-[#9BA3AF] group-hover:bg-[#EAECEF]"
                                  }`}
                                >
                                  <ChevronDown
                                    size={18}
                              ...
                              {/* Elegant Separator */}
                              {!isActive &&
                              index !== policyAccordion.length - 1 ? (
                      ...
                      return (
                        ...
                        >
                          ...
                          <div className="absolute -top-4 left-5 flex h-10 w-10 items-center justify-center rounded-xl bg-gradient-to-br from-[#FF8A4C] to-[#ff7029] text-white shadow-[0_8px_16px_rgba(255,138,76,0.4)] ring-4 ring-white">
                            {React.createElement(notice.icon || Sparkles, {
                              size: 18,
                ...
                <div className="flex flex-col gap-12 lg:flex-row lg:items-center xl:gap-16">
                  <div className="lg:w-[45%]">
                    ...
                    <h3 className="text-3xl font-black leading-[1.15] tracking-tight text-white sm:text-4xl lg:text-5xl drop-shadow-sm">
                      A safer, calmer <br className="hidden sm:inline" /> spa
                      experience.
                    </h3>
                      ...
                      return (
                        ...
                        >
                          <div className="inline-flex h-12 w-12 items-center justify-center rounded-[1rem] bg-[#FFF7F2] text-[color:var(--orange)] shadow-inner ring-1 ring-[rgba(255,138,76,0.15)] group-hover:scale-110 group-hover:bg-[color:var(--orange)] group-hover:text-white transition-all duration-300">
                            {React.createElement(item.icon || ShieldCheck, {
                              size: 22,
                              strokeWidth: 2.5,
                            })}
                          </div>
            ...
            <div className="mx-auto w-full max-w-4xl relative z-10 px-4 sm:px-6">
              <div className="text-center mb-10 sm:mb-12">
                ...
                <h2 className="text-3xl sm:text-[40px] font-black tracking-tight text-[color:var(--navy)] leading-tight">
                  Quick answers for <br className="hidden sm:inline" /> pet
                  parents
                </h2>
                <p className="mt-4 text-[15px] sm:text-[16px] text-[color:var(--text)] opacity-80 max-w-2xl mx-auto font-medium leading-relaxed">
                  Find out everything you need to know about our policies,
                  procedures, and how we ensure the best care for your furry
                  friends.
                </p>
              </div>
                  ...
                  return (
                    <div
                      ...
                      className={`group relative overflow-hidden rounded-[1.25rem] transition-all duration-300 bg-white ring-1 ring-[rgba(228,232,238,0.8)] shadow-[0_2px_10px_rgba(30,42,58,0.02)] hover:shadow-[0_8px_30px_rgba(30,42,58,0.06)] hover:ring-[rgba(228,232,238,1)] ${
                        isActive
                          ? "ring-[rgba(255,138,76,0.3)] shadow-[0_8px_30px_rgba(255,138,76,0.08)]"
                          : ""
                      }`}
                    >
                      <button
                      ...
                      >
                        <div className="flex items-center gap-4 flex-1 pr-6">
                          <div className="flex h-10 w-10 sm:h-11 sm:w-11 shrink-0 items-center justify-center rounded-xl bg-[#F8FAFC] text-[color:var(--text)] opacity-70 transition-all duration-300 group-hover:bg-[#F1F5F9] group-hover:text-[color:var(--navy)] group-hover:opacity-100">
                            <Sparkles
                              size={20}
                              className={
                                isActive ? "text-[color:var(--orange)]" : ""
                              }
                            />
                          </div>
                        ...
                        <div
                          className={`shrink-0 text-[#94A3B8] transition-transform duration-300 ${isActive ? "rotate-180 text-[color:var(--orange)]" : ""}`}
                        >
              ...
              <div className="relative z-10 flex flex-col gap-8 md:flex-row md:items-center md:justify-between px-4 sm:px-8">
                <div className="md:max-w-md lg:max-w-lg">
                  <div className="mb-4 inline-flex items-center gap-2 rounded-full bg-white/10 px-3.5 py-1.5 backdrop-blur-md ring-1 ring-white/20 shadow-md shadow-black/10">
                    <span className="policy-kicker mb-0 text-white/90 font-semibold tracking-wider">
                      SUPPORT TEAM
                    </span>
                  </div>
